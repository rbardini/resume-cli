...
function isUpToDate(callback) {
  ...
  request.get('https://registry.npmjs.org/@rbardini/resume-cli', function(err, res) {
    if (err) {
      return callback(err);
    }
    ...
    if (localVersion === latestnpmVersion) {
      ...
      console.log(chalk.cyan('Type'), '`npm install @rbardini/resume-cli@latest`', chalk.cyan('to upgrade to version'), latestnpmVersion);
    }
