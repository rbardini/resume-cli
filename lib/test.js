...
function errorFormatter(errors) {
    ...
    errors.forEach(function(error) {
        console.log('    ', cross, chalk.gray(pathFormatter(error.path), error.message));
    });
...
function validate(resumeData, callback) {
    ...
    resumeSchema.validate(resumeData, function(errs, report) {
        if (errs) {
            ...
            // console.log(report)
            console.log('To publish your resume at'.cyan, 'https://jsonresume.org', 'type the command'.cyan, 'resume publish');
